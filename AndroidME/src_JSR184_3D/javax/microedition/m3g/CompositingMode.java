...
//JP		gl.glBlendEquation(GL11.GL_FUNC_ADD);
		gl.glBlendFunc(GL11.GL_ADD, 0);
		...
		if (alphaThreshold > 0)
		{
			gl.glAlphaFunc(GL11.GL_GEQUAL, alphaThreshold);
		...
		if (blending != REPLACE)
		{
			switch (blending)
			{
				...
				case ALPHA:
					...
					break;
				case MODULATE:
					gl.glBlendFunc(GL11.GL_DST_COLOR, GL11.GL_ZERO);
					break;
				case MODULATE_X2:
		...
		if (depthOffsetFactor != 0 || depthOffsetUnits != 0)
		{
			...
			gl.glDisable(GL11.GL_POLYGON_OFFSET_FILL);
	}
}
