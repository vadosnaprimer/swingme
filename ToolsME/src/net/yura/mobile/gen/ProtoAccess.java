...
public class ProtoAccess extends ProtoUtil
{
    ...
    public void writeTest( net.yura.tools.mobilegen.model.Test obj , ProtoOutputStream out ) throws IOException
    {
         ...
         if ( obj != null )
         {
             // FIELD : id
             // SINGLE INSTANCE OF PRIMITIVE FIELD.
             protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_TEST_ID , obj.getId() );
    ...
    public void writeNumber( net.yura.tools.mobilegen.model.Number obj , ProtoOutputStream out ) throws IOException
    {
         ...
         if ( obj != null )
         {
             // FIELD : number
             // SINGLE INSTANCE OF PRIMITIVE FIELD.
             protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_NUMBER_NUMBER , obj.getNumber() );

             // FIELD : type
             // SINGLE INSTANCE OF PRIMITIVE FIELD.
             protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_NUMBER_TYPE , obj.getType() );
        ...
        for( Enumeration keys = hashtableOfObjectArraysAsVectors.keys() ; keys.hasMoreElements() ; )
        {
            ...
            if ( temporaryVector != null )
            {
                if ( fieldName.equals( "legs" ) )
                {
                    ...
                    objTestObject.setLegs( temporaryObjectArray );
                }
            }
        }
    ...
    public void writeTestObject( net.yura.tools.mobilegen.model.TestObject obj , ProtoOutputStream out ) throws IOException
    {
         ...
         if ( obj != null )
         {
             // FIELD : id
             // SINGLE INSTANCE OF PRIMITIVE FIELD.
             protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_TESTOBJECT_ID , obj.getId() );

             // FIELD : name
             // SINGLE INSTANCE OF PRIMITIVE FIELD.
             protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_TESTOBJECT_NAME , obj.getName() );

             // FIELD : age
             // SINGLE INSTANCE OF PRIMITIVE FIELD.
             protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_TESTOBJECT_AGE , obj.getAge() );
             ...
             if ( v != null )
             {
                 ...
                 for( ; e.hasMoreElements() ; )
                 {
                     ...
                     writeNumber( (net.yura.tools.mobilegen.model.Number)e.nextElement() , new ProtoOutputStream( baos ) );
                     buffer = baos.toByteArray();
                     baos.close();
                     baos   = null;
                     length = _out.encodeVariableInteger((long)buffer.length);
                     id     = _out.encodeVariableInteger( _out.encodeFieldIndexAndWireFormatType( NET_YURA_TOOLS_MOBILEGEN_MODEL_TESTOBJECT_NUMBERS , ProtoInputStream.WIRE_FORMAT_LENGTH_DELIMITED ) );
                     _out.write( id );
                     _out.write( length );
                     _out.write( buffer );      
                     id     = null;
             ...
             // OBJECT
             baos = new ByteArrayOutputStream();
             writeObject( obj.getBody() , new ProtoOutputStream( baos ) );
             buffer = baos.toByteArray();
             baos.close();
             baos   = null;
             _out.write( buffer );      
             ...
             for( int arrayIndex = 0 ; arrayIndex < tmpArray.length ; arrayIndex++ )
              {
                 protoOutputStream.write( NET_YURA_TOOLS_MOBILEGEN_MODEL_TESTOBJECT_LEGS , tmpArray[ arrayIndex ] );
             } 
