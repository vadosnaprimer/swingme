...
//    Deserializes Object3D instances from the named resource.
		try {
			return new Loader(name).load(ApplicationManager.getInstance().openInputStream(name));
		} catch (IOException e) {
	...
	private void readSection(M3gInputStream is) throws IOException{
		...
		while(true){
			...
			case 255:
				...
				uri = ApplicationManager.concatPath(this.uri, uri);
				
				
				object = new Loader(uri).load(ApplicationManager.getInstance().openInputStream(uri))[0];
