...
public class CallStack
{
  ...
  private DataOutputStream
    stream = new DataOutputStream(stacktrace);
  ...
  public byte[] createStacktrace()
  {
    CallStack callStack = getCallStack();
    callStack.writeStacktrace(Thread.currentThread());
		return callStack.stacktrace.toByteArray();
  }

	public static byte[] getStacktrace()
	{
  ...
  void writeStacktrace(Object message)
	{
    try
    {
      stacktrace.reset();
      stream.writeUTF(message.toString().replace('\n', ' '));
      StringBuffer args = new StringBuffer();
      for(int intPtr = this.intPtr, objPtr = this.objPtr; intPtr>0;)
      {
        stream.writeInt(intStack[--intPtr]);
        stream.writeShort(line);
        line = intStack[--intPtr]>>16;
        args.setLength(0);
...
//            break;
	...
	private static class MonitoredCallStack extends CallStack
	{
		...
		private void saveStacktrace()
		{
			try
			{
        writeStacktrace(Thread.currentThread());
				byte[] bytes = stacktrace.toByteArray();
